...
//................................  mouseup
	...
	$("*").on("mouseup", function(event) { mouseIsDown = false });
...
function createSession() {
	...
	var eventIds = '';
	for (var i = eventIndex; i < sessions.length; i++) {
...
if (event[TYPE].match(/resize/) || initFrame) {
		$("#screenshot-frame, #screenshot, #screenshot-back, #screen").css({"width":sessions[eventIdOrPng][WIN_INNER_WIDTH] * reducFactor, "height":sessions[eventIdOrPng][WIN_INNER_HEIGHT] * reducFactor});
		initFrame = false;
}
...
function displayEventMobile() {
	...
	$("#screenshot, #screenshot-back, #screenshot-frame").css({"opacity":1}); // pour fin resize
...
//...  MISE À L'ÉCHELLE
	...
	$("#screenshot-frame, #screen").css({"width":$("#screenshot").width(), "height":$("#screenshot").height()});
...
//*****************************************************************************************
	...
	function zoomGesture(eventId) {
		...
		lastImg.animate({"width":lastImg.width() * zoomMul }, startEventCountDown, 'linear');
	}
//*****************************************************************************************
...
//		if (!x && !y) return false;
		...
		var opacity;
		if (event[TYPE].match(/Gesture|touch|click|mouse|scroll/)) opacity = 0.9;
		else opacity = 0;
...
//*************************************************************************************************
	...
	//	$("#timeline").css({"top":(TOP_BOARD_HEIGHT * 3) + (BOARD_MARGIN * 14) + 4 + $("#screenshot").height()});
		...
		if ((event[TARGET] == 'TextArea') || (event[TARGET] == 'Input') || (event[TYPE] == 'input')) $("#top-board3").css({"color":"white", "background-color":"black", "font-size":24});
		else $("#top-board3").css({"color":"black", "background-color":VERY_LIGHT_GRAY, "font-size":18});
	...
	//	A F F I C H A G E 	MINI DOC      			*******************************************
		...
		if (isMobile()) {
			...
			zoomIndirectFactor = pngIndirect[ZOOM];
		}
...
/*		if (preAnimTime < 0) {
		...
		waitResize1 = setTimeout(function () {
			...
			var totalTransImgX = totalTransImgY = 0;
		
			var i = 3;
			$("#screenshot, #screenshot-back, #screenshot-frame").css({"opacity":0});
			...
			inter1 = setInterval(function() {
				...
				if (i >= animCount) {
					...
					$("#resize-canvas").remove();
					//$("#screenshot, #screenshot-back, #screenshot-frame").css({"opacity":1});
				}
			}, animTime);
		}, preAnimTime);
