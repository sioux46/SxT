...
//	$result=$idcom->query($requete);
    if(!$result) {
    	...
    	while ($row = $result->fetch_array(MYSQLI_NUM)) {
			...
			$value = $row[0];
    		foreach($row as $donn) {
    			$ligne = $ligne . utf8_encode($donn) . " ";
    		}
			$r++;
			if (($numRows == $r) && ($sel == "sel")) $selected = "selected";
			else  $selected = "";
				
			echo "<option $selected value=", $value, ">", $ligne, "</option>\n";
    	}
		echo "</select>";
//    	$result->close();
    }
...
function echoResultTable($result, $colTitles) {
    if(!$result) {
		...
		echo '<div style="overflow:auto; height:800px;">';
		echo "<table>";
		if ($colTitles) {
...
function echoResultTablePng($result, $colTitles, $idcom, $colPng, $resultPngInfo, $reducShowPng) {
    if(!$result) {
		...
		if ($colTitles) {
			...
			echo '<th>', utf8_encode($colonne->name), '</th>';
		    }
		}
    	while ($row = $result->fetch_array(MYSQLI_NUM)) {
		...
		if ($colPng == 0) $stop = 1;
    		foreach($row as $donn) {
		    if ($stop) {$stop = 0; continue;}
	$donn = preg_replace('/<!--/','<--',$donn);   // virer les comm. html
	$donn = htmlentities($donn);   //  literal html
	echo '<td>', utf8_encode($donn), '</td>';
    		}
    		echo '</tr>';
...
//				logbug($requetePng);
				...
				if ($png && $png[0][0] == '<') {
					...
					echo "<script> displayPng($divIndex,$rowPngInfo[0],$rowPngInfo[1],$rowPngInfo[2],$rowPngInfo[3],$rowPngInfo[4],$rowPngInfo[5],$rowPngInfo[6],\"$rowPngInfo[7]\",$reducShowPng);</script>";
...
//////////////////////////////////////////////////////////////
////////////////		SUPPRESSION DE SESSIONS MUET
/////////////////////////////////////////////////////////////
function deleteSessionMuet($sessionId, $idComW) {
